 ...
 * - pi/qwen2.5:14b: Token-heavy simple tasks (free local Ollama)
 */
...
const MODEL_PRICING = {
  ...
  'pi': { input: 0.00, output: 0.00 }, // Free local (logical name)
  'qwen2.5:14b': { input: 0.00, output: 0.00 }, // Free local (Ollama)
  'ollama': { input: 0.00, output: 0.00 } // Free local (generic)
};
...
function selectLLM(taskSpec) {
  ...
  // PI-MONO: Token-heavy simple tasks (free local)
    ...
    execution = 'pi';
    reason = 'Token-heavy simple task - use free local Ollama (qwen2.5:14b)';
  }
