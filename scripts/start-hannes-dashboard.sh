...
# Hard isolation: explicitly force Hannes runtime values regardless inherited shell/launchd env.
export AGENT_API_URL="http://localhost:${PORT}"
export REPOS_CONFIG_PATH="$SERVICE_ROOT/config/repos.hannes.yaml"
export AGENT_DB_PATH="$SERVICE_ROOT/.compound-state/agent-service-hannes.db"
export APP_DISPLAY_NAME="Hannes AI"
export OPENCLAW_CONFIG_PATH="$HOME/.openclaw-hannes/openclaw.json"
export OPENCLAW_STATE_DIR="$HOME/.openclaw-hannes/state"
...
export AGENT_JOBS_DIR="$SERVICE_ROOT/.compound-state/running-jobs-hannes"
export AGENT_SESSION_LOGS_DIR="$SERVICE_ROOT/.compound-state/orchestrator-sessions-hannes"
