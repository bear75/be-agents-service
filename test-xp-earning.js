...
console.log('='.repeat(60));
console.log('Level:', agentGamification.level, agentGamification.level_emoji, agentGamification.title);
...
if (agentGamification.recentXP && agentGamification.recentXP.length > 0) {
  ...
  agentGamification.recentXP.forEach(tx => {
    console.log(`  ${tx.amount > 0 ? '+' : ''}${tx.amount} XP - ${tx.reason} (${tx.source_type})`);
  });
}
