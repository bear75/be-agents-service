...
Under **iCloud/AgentWorkspace/** you have one folder per context (e.g. **DARWIN**, **default**, or a repo/project name). That folder is the **single source of truth**: inbox, memory, priorities, agent-reports. Cursor, compound scripts, and Telegram/OpenClaw all read and write there.
...
**Right:** Set `agent.workspace` (or the main agent’s `workspace` in `agents.list`) to the **exact same** AgentWorkspace path that `config/repos.yaml` uses for that context (e.g. `AgentWorkspace/DARWIN`), or `AgentWorkspace/default` if you use one generic workspace.
...
- Send `status` or `overview` to the Telegram bot; it should report the workspace path that is your **AgentWorkspace** folder (e.g. DARWIN, default, or your project name).
...
- Telegram setup: `scripts/setup-telegram-openclaw.sh`
